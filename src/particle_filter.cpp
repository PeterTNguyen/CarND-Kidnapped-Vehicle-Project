    ...
    const std::vector<LandmarkObs> &observations, const Map &map_landmarks) 
{
    ...
    for(vector<Particle>::iterator particle = particles.begin(); particle != particles.end(); ++particle)
    {
        // New Vector for transformed map observations
        vector<LandmarkObs> mapObsVec;
        LandmarkObs mapObs;
        ...
        for(vector<LandmarkObs>::const_iterator obs = observations.begin(); obs != observations.end(); ++obs)
        {
            ...
            mapObsVec.push_back(mapObs);

        }

        //Data Association(map landmark locations, transformed observations)
        dataAssociation(map_landmarks, mapObsVec);
        ...
        // Go through each tramsformed map Observation and calculate a weight
        for(vector<LandmarkObs>::iterator obs = mapObsVec.begin(); obs != mapObsVec.end(); ++obs)
        {
            //Multi-variate gaussian and weight calculation
        ...
        //Update particle weight and keep track of max weight
...
void ParticleFilter::resample() 
{
    ...
    for(int i = 0; i < num_particles; i++)
    {
        //Add uniformly random variable [0, 2*max_weight] to beta
        beta += rand_weight( gen );

        //Keep iterating through weights until a weight > beta
